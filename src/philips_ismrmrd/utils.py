"""Utility functions for reading and processing Philips MR data files."""
...
def _validate_path(path_to_file: str, expected_extension: str):
    ...
    if Path(path_to_file).stat().st_size == 0:
        raise ValueError(f'File is empty: {path_to_file}')


def _extract_general_info(list_lines):
    general_info = [line for line in list_lines if line.startswith(('#', '.'))]
    if not general_info:
...
def _extract_attributes(list_lines):
    attribute_lines = [line for line in list_lines if not line.startswith(('#', '.'))]
...
def _read_and_store_samples_per_type(samples: dict, path_to_datafile: str, attributes: pd.DataFrame):
    with Path(path_to_datafile).open(mode='rb') as f:
